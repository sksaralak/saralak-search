...
const problems = [
  'บางเว็บไซต์ไม่ได้ขาดบทความ แต่ขาด structure ที่ช่วยให้ Google เข้าใจ',
  'บางเว็บไซต์มี traffic แต่ไม่เกิด lead หรือ inquiry',
  'บางธุรกิจมี Google Maps แล้ว แต่ profile ยังไม่ช่วยให้ลูกค้าโทรหรือเดินทางมา',
  'บางแบรนด์มี content เยอะ แต่ AI Search ยังไม่เข้าใจว่าเชี่ยวชาญเรื่องอะไร',
]

const checks = [
  { label: 'Google visibility — เว็บไซต์ถูกค้นเจอจาก keyword สำคัญแค่ไหน', icon: <svg className="h-4 w-4 shrink-0 text-teal-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg> },
  { label: 'Competitor gap — คู่แข่งได้ traffic หรือ lead จากจุดไหน', icon: <svg className="h-4 w-4 shrink-0 text-teal-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 013 19.875v-6.75zM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V8.625zM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V4.125z" /></svg> },
  { label: 'Content gap — เว็บไซต์ตอบ search intent และ business intent ครบหรือยัง', icon: <svg className="h-4 w-4 shrink-0 text-teal-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M2.25 12l8.954-8.955c.44-.439 1.152-.439 1.591 0L21.75 12M4.5 9.75v10.125c0 .621.504 1.125 1.125 1.125H9.75v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21h4.125c.621 0 1.125-.504 1.125-1.125V9.75M8.25 21h8.25" /></svg> },
  { label: 'Technical SEO — ปัญหา indexing, sitemap, internal link หรือ page structure', icon: <svg className="h-4 w-4 shrink-0 text-teal-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z" /></svg> },
  { label: 'Local visibility — Google Business Profile และข้อมูลสาขาพร้อมแค่ไหน', icon: <svg className="h-4 w-4 shrink-0 text-teal-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M15 10.5a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z" /></svg> },
  { label: 'AI visibility — AI Search เข้าใจ brand, service, expertise และ entity ชัดหรือยัง', icon: <svg className="h-4 w-4 shrink-0 text-teal-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09z" /></svg> },
  { label: 'CTA readiness — แต่ละหน้าพาคนอ่านไปสู่ lead ได้ดีแค่ไหน', icon: <svg className="h-4 w-4 shrink-0 text-teal-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M2.25 18L9 11.25l4.306 4.307a11.95 11.95 0 015.814-5.519l2.74-1.22m0 0l-5.94-2.28m5.94 2.28l-2.28 5.941" /></svg> },
]

const deliverables = [
  { label: 'ภาพรวมว่าเว็บไซต์กำลังเสียโอกาสตรงไหน', icon: <svg className="h-4 w-4 shrink-0 text-teal-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" /></svg> },
  { label: 'จุดที่ควรแก้ก่อนตาม priority', icon: <svg className="h-4 w-4 shrink-0 text-teal-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 6.75V15m6-6v8.25m.503 3.498l4.875-2.437c.381-.19.622-.58.622-1.006V4.82c0-.836-.88-1.38-1.628-1.006l-3.869 1.934c-.317.159-.69.159-1.006 0L9.503 3.252a1.125 1.125 0 00-1.006 0L3.622 5.689C3.24 5.88 3 6.27 3 6.695V19.18c0 .836.88 1.38 1.628 1.006l3.869-1.934c.317-.159.69-.159 1.006 0l4.994 2.497c.317.158.69.158 1.006 0z" /></svg> },
  { label: 'โอกาส keyword / content / Google Maps / AI Search', icon: <svg className="h-4 w-4 shrink-0 text-teal-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M2.25 18L9 11.25l4.306 4.307a11.95 11.95 0 015.814-5.519l2.74-1.22m0 0l-5.94-2.28m5.94 2.28l-2.28 5.941" /></svg> },
  { label: 'คำแนะนำที่เชื่อมกับเป้าหมายธุรกิจ ไม่ใช่แค่ checklist SEO', icon: <svg className="h-4 w-4 shrink-0 text-teal-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 18v-5.25m0 0a6.01 6.01 0 001.5-.189m-1.5.189a6.01 6.01 0 01-1.5-.189m3.75 7.478a12.06 12.06 0 01-4.5 0m3.75 2.383a14.406 14.406 0 01-3 0M14.25 18v-.192c0-.983.658-1.823 1.508-2.316a7.5 7.5 0 10-7.517 0c.85.493 1.509 1.333 1.509 2.316V18" /></svg> },
  { label: 'แนวทางเลือก service ที่เหมาะ — SEO, Local SEO, GEO หรือ Content', icon: <svg className="h-4 w-4 shrink-0 text-teal-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M3.75 12h16.5m-16.5 3.75h16.5M3.75 19.5h16.5M5.625 4.5h12.75a1.875 1.875 0 010 3.75H5.625a1.875 1.875 0 010-3.75z" /></svg> },
  { label: 'Recommendation ที่เอาไป brief ทีมเว็บหรือทีม content ต่อได้เลย', icon: <svg className="h-4 w-4 shrink-0 text-teal-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 002.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 01-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 00-1.091-.852H4.5A2.25 2.25 0 002.25 4.5v2.25z" /></svg> },
]
...
const pricingPlans = [
  {
    tier: 'Starter',
    price: '5,000 THB',
    fit: 'เหมาะสำหรับเว็บไซต์ขนาดเล็กหรือธุรกิจที่ต้องการรู้ปัญหาหลักก่อนเริ่มทำ SEO',
  ...
  },
  {
    tier: 'Growth',
    price: '12,000 THB',
    fit: 'เหมาะสำหรับธุรกิจที่มีเว็บไซต์, content หรือหลายบริการ และต้องการ roadmap ที่ละเอียดขึ้น',
  ...
  },
  {
    ...
    fit: 'เหมาะสำหรับเว็บไซต์ขนาดใหญ่ หลายภาษา หลายสาขา หรือมีหลายทีมที่เกี่ยวข้อง',
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-neutral-800 bg-neutral-900 shadow-2xl shadow-black/50">
      <div className="border-b border-neutral-800 bg-neutral-950 px-6 py-4">
        <p className="font-mono text-sm font-semibold tracking-wider text-teal-400">DISCOVERY AUDIT REPORT</p>
        <p className="mt-1 font-mono text-sm text-neutral-400">your-brand.com · Jun 2026 · Saralak Search</p>
      </div>

      <div className="border-b border-neutral-800 px-6 py-5">
        <div className="flex items-center justify-between">
          <p className="font-mono text-sm text-neutral-400">Brand Visibility Score</p>
          <p className="font-mono text-2xl font-black text-amber-400">4.2<span className="text-sm text-neutral-400">/10</span></p>
        </div>
        ...
        <p className="mt-2 font-mono text-sm text-neutral-400">3 critical gaps · 8 quick wins available</p>
      </div>

      <div className="border-b border-neutral-800 px-6 py-4">
        <p className="mb-3 font-mono text-sm font-semibold uppercase tracking-wider text-neutral-400">Platform Analysis</p>
        <div className="grid gap-2.5">
          {[
            ...
            <div key={p} className="flex items-center gap-3">
              <span className={`shrink-0 text-sm ${txt}`}>{icon}</span>
              <span className="w-28 shrink-0 font-mono text-sm text-neutral-300">{p}</span>
              <div className="h-1 flex-1 rounded-full bg-neutral-800">
                <div className={`h-1 rounded-full ${bar_c}`} style={{ width: `${bar}%` }} />
              </div>
              <span className={`shrink-0 font-mono text-sm font-bold ${txt}`}>{score}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="px-6 py-5">
        <p className="mb-3 font-mono text-sm font-semibold uppercase tracking-wider text-neutral-400">Priority Actions</p>
        <div className="grid gap-2">
          {[
            ...
            <div key={item.text} className="flex items-start gap-2.5">
              ...
              <span className="font-mono text-sm text-neutral-300">{item.text}</span>
            </div>
          ))}
        </div>
        <p className="mt-4 font-mono text-sm text-teal-400">→ Full roadmap: 12 prioritized actions ready</p>
      </div>
    </div>
  )
}
    ...
    <main>
          ...
          <div className="grid items-center gap-12 lg:grid-cols-2">
            <div>
              ...
              <p className="thai-readable mt-6 text-lg leading-8 text-neutral-300">
                ...
                คู่แข่งได้เปรียบตรงไหน และควรเริ่มแก้จุดไหนก่อน
                เพื่อให้การลงทุน SEO, GEO หรือ Google Maps คุ้มค่าที่สุด
              </p>
      ...
      <section className="border-b border-neutral-200 bg-white">
        <div className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
          <p className="text-sm font-semibold uppercase tracking-wide text-teal-800">ปัญหาที่ Audit ช่วยตอบ</p>
          <h2 className="thai-readable mb-8 mt-3 text-2xl font-semibold text-neutral-950">หลายธุรกิจเริ่มทำ SEO โดยยังไม่รู้ว่าปัญหาจริงอยู่ตรงไหน</h2>
        ...
        <div className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
          <div className="grid gap-12 lg:grid-cols-2">
            ...
            <div>
              ...
              <h2 className="mt-3 text-2xl font-semibold text-white">Discovery Audit ตรวจอะไรบ้าง</h2>
            ...
            <div>
              ...
              <h2 className="mt-3 text-2xl font-semibold text-white">สิ่งที่คุณจะได้รับ</h2>
              <div className="mt-8 grid gap-3">
      ...
      <section className="border-t border-neutral-200 bg-white">
        <div className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
          <SectionHeader
            ...
            description="เหมาะสำหรับธุรกิจที่อยากลงทุนให้ถูกจุด — รู้ก่อนว่าปัญหาอยู่ที่ SEO, Google Maps หรือ AI Search แล้วค่อยเลือก service"
          />
              ...
              plan.featured ? (
                <article key={plan.tier} className="flex flex-col overflow-hidden rounded-2xl bg-gradient-to-br from-teal-950 via-[#0d3d36] to-neutral-900 text-white shadow-2xl shadow-teal-950/40">
                  ...
                  <div className="mt-auto px-7 pb-7 pt-6">
                    <CTAButton to="/contact" className="w-full">ขอ Growth Audit</CTAButton>
                  </div>
                </article>
              ) : (
